...
```sh
ubuntu@VM-0-17-ubuntu:~/3proxy-0.9.4$ sudo make -s install
update-rc.d: error: no runlevel symlinks to modify, aborting!
scripts/debian/postinst: 10: /usr/sbin/chkconfig: not found
...
Created symlink /etc/systemd/system/3proxy.service → /lib/systemd/system/3proxy.service.
Created symlink /etc/systemd/system/multi-user.target.wants/3proxy.service → /lib/systemd/system/3proxy.service.
...
It's recommended to use provider supplied DNS or install local recursor, e.g. pd                                                                                                                                                                             ns-recursor.
...
ubuntu@VM-0-17-ubuntu:~/3proxy-0.9.4$

```

sudo service 3proxy status   
Ubuntu 22.04  缺少 /var/run/3proxy/ 及权限 创建不了pid文件.  
解决方案:  
vi /etc/3proxy/3proxy.cfg
system "mkdir /var/run/3proxy || true"
